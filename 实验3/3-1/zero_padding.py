...
def get_1D_chirp(x, phi):
    return 10 * np.cos(8 * np.pi * (x - x0)**2 + phi)
...
# 0填充方法
signal_trans = [0 for _ in range(max((len(signal) - x0 * fs)**2, (x0 * fs)**2))]
for i, sig in enumerate(signal):
    signal_trans[(i - x0 * fs)**2] = sig

t2 = [i / fs for i in range(len(signal_trans))]
...
# 忽略 0 频率附近的区间
threshold = 0.5                         # 忽略频率绝对值小于 1 Hz 的数据
