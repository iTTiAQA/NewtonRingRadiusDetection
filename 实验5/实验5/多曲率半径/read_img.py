...
# 指定你的图片文件夹路径
folder_path = 'D:/湖南大学/3.全代码/codelabs中进行深度学习训练及估计/Ideal/train/'
# 指定输出的CSV文件路径（注意要指定完整文件路径和文件名）
csv_file_path = 'D:/湖南大学/3.全代码/codelabs中进行深度学习训练及估计/Ideal/train/data.csv'

# 曲率半径实际值,可以替换为你的牛顿环曲率半径
actual_radius = 0.85
...
with open(csv_file_path, mode='w', newline='', encoding='utf-8') as csv_file:
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp')):
            ...
            # 计算归一化后的曲率半径
            normalized_radius = actual_radius

            # 写入文件名和归一化后的曲率半径到CSV
            filename = './Ideal/train/' + filename
